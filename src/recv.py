...
def main() -> None:
    ...
    print(describe_calibration(args.mode, calibrate_receiver(
        endpoint, args.mode, args.timeout, CALIBRATION_WAIT_S, enabled=calibrate
    )))
    progress = ProgressLine()
    start = time.monotonic()
    last_activity = start
    # Свой ответ в эфире (NACK на сотни кадров — это десятки секунд) молчанием
    # отправителя не считаем: полудуплекс, в это время нас и не должны звать.
    air_time = 0.0
    ...
    while True:
        ...
        now = time.monotonic()
        if not got:
            if now - last_activity - air_time > args.timeout:
                progress.finish()
